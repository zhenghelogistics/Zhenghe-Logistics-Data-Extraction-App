...
const USER_TEXTS = {
  ...
  logistics: "This PDF contains logistics documents — it may include multiple Shipping Instructions from PULAU SAMBU SINGAPORE. STEP 1: Scan EVERY single page — do not stop early. STEP 2: For EACH Shipping Instruction (2-page document on PULAU SAMBU SINGAPORE letterhead with a SHIPPING INSTRUCTION header), create ONE 'Outward Permit Declaration' entry. Get container_no and seal_no from the 'FOR SHIPPING DEPARTMENT ONLY' section on page 2. STEP 3: For each SI where the 'Documents Required' field on page 1 contains the words 'Export Declaration permit' or 'Export Permit', ALSO create a SECOND entry for that same SI as 'Export Permit Declaration (PSS)'. For this PSS entry, populate export_permit_pss.items — one item per product line in the SI's product table — using EXACTLY these field names: item_description (product name), hs_code (from HS table or SI), quantity (integer, no decimals), uom (unit e.g. Cartons, Bags), amount (line total, number only), currency (e.g. USD), po_number (purchase order from SI), invoice_number (SI reference number). Do NOT leave export_permit_pss.items empty. STEP 4: Output one OPD entry per SI (plus PSS where applicable) — do NOT skip any SI, do NOT combine. CRITICAL: Respond ONLY with valid JSON using EXACTLY this structure: {\"documents\": [...]}. No explanation, no markdown.",
