...
- HOW TO IDENTIFY AN SI: An SI is any document (regardless of letterhead) whose second page has a "FOR SHIPPING DEPARTMENT ONLY" section with fields for Container No, Seal No, Vessel, Voyage, and Booking Ref. The letterhead may read "PULAU SAMBU SINGAPORE", "RSUP", or any other exporter name — ALL are Outward Permit Declarations.
- DATA ISOLATION BETWEEN SIs: Each SI owns its own product descriptions, consignee names, quantities, and values. The anchor for each OPD entry is the container number in that SI's "FOR SHIPPING DEPARTMENT ONLY" section. Do NOT carry over products or quantities from a different SI. If product data in your current chunk is ambiguous about which SI it belongs to, assign it to the SI whose "FOR SHIPPING DEPARTMENT ONLY" section it is physically closest to. This rule does NOT restrict reading the shared BL Draft summary page (for BL number) or the packing list pages that clearly belong to the current SI.
...
- PENDING / UNCONFIRMED BOOKINGS: If the "FOR SHIPPING DEPARTMENT ONLY" section shows "/" or is blank for container_no and/or seal_no, set those fields to null — do NOT skip the SI. The shipment is real; the booking is just not yet confirmed. Skip only pages that are clearly pre-entry placeholder stubs: no product lines, no consignee, and no "FOR SHIPPING DEPARTMENT ONLY" section at all.
- MULTI-SHIPPER CONTAINERS (PSG + RSUP sharing the same container): Some SIs carry goods from TWO exporters in the same container. This may appear as:
  (A) Two separate SI documents in the batch — one PSG/PULAU SAMBU SINGAPORE SI and one RSUP SI — both showing the same container number in their respective "FOR SHIPPING DEPARTMENT ONLY" sections. Create one OPD entry per document (two total for the same container).
  (B) One SI document with a main PSG section AND a separate RSUP shipper sub-section within the same pages. Create TWO OPD entries from this one SI, sharing the same container_no, seal_no, vessel, and voyage. Each entry has its own exporter, consignee, description, hs_code, quantities, and values from its respective section.
  In both cases: NEVER skip the RSUP portion. The RSUP section will be clearly labelled "RSUP" or show a different exporter name from "PULAU SAMBU SINGAPORE".
