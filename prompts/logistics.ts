...
- HOW TO IDENTIFY AN SI: An SI is any document whose second page has a "FOR SHIPPING DEPARTMENT ONLY" section with fields for Container No, Seal No, Vessel, Voyage, and Booking Ref. The exporter letterhead does not matter — an SI from any company is an Outward Permit Declaration.
...
- MULTI-SHIPPER CONTAINERS (two exporters sharing the same container): Some containers carry goods from two different exporters. This may appear as:
  (A) Two separate SI documents in the batch — from different exporters — both showing the same container number in their respective "FOR SHIPPING DEPARTMENT ONLY" sections. Create one OPD entry per SI document (two total for the same container).
  (B) One SI document that contains two distinct shipper sections within the same pages, each with their own exporter, consignee, products, and values. Create TWO OPD entries from this one SI, sharing the same container_no, seal_no, vessel, and voyage. Each entry has its own exporter, consignee, description, hs_code, quantities, and values from its respective section.
  In both cases: extract EVERY shipper section — do not skip any.
