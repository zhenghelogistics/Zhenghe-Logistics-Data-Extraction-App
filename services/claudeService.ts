...
- MULTI-INVOICE RULE:
  ...
  * CRITICAL: For EVERY bl_entry, you MUST scan the ENTIRE document for its pss_invoice_number — do not leave it null if the number exists anywhere in the document. Each BL line item in a batch invoice often has its own SI/reference number printed next to it (e.g. in a column, in brackets, or in a remarks field). Look for labels like "SI:", "Invoice:", "Inv No:", "Ref:", "PO:" next to each BL row. All entries in the same batch document typically share the same SI number format — if you found it for some entries, look harder for the rest.
